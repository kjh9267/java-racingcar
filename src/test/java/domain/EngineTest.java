...
public class EngineTest {
    ...
    void constructorTest() {
        ...
        assertThat(engine)
                ...
                .isInstanceOf(Engine.class);
    }

    @DisplayName("엔진 작동 판단 기능.")
    @ParameterizedTest
    @MethodSource("provideInputForCanGo")
    void canRunningTest(final int input, final boolean expected) {
    ...
    void canRunningTest2(final int input) {
        assertThatThrownBy(() -> engine.canRunning(input))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("유효한 값이 아닙니다 : %d", input);
    }

    @DisplayName("엔진이 run 할때 running count가 1씩 상승하는지 확인")
    @Test
    void runTest() {
        // Given
        final int initialRunningCount = engine.getRunningCount();
        ...
        // Then
        assertThat(engine.getRunningCount()).isEqualTo(initialRunningCount + 2);
    }
}
